    ...
    def get_current_user(sso_client, request):
        ...
        try:
            ...
            if res.ok:
                ...
                res_unis = sso_client.get('api/unis', token=OAuth2Token(token))
                if res_unis.ok:
                    request.session['unis'] = res_unis.json()
                res_reg = sso_client.get('api/registration', token=OAuth2Token(token))
                if res_reg.status_code == 200:
                    reg = res_reg.json()
                    request.session['uni'] = reg['uni_id']
                    request.session['confirmed'] = reg['confirmed']
                else:
